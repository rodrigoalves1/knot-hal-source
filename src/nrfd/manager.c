...
static int peers_to_json(struct json_object *peers_bcast_json)
{
	...
	while (g_hash_table_iter_next (&iter, &key, &value)) {
		...
		json_object_object_add(jobj, "mac",
					json_object_new_string(peer->mac_str));
		json_object_object_add(jobj, "time_last_bcast",
				json_object_new_int(peer->time_last_bcast));
...
static int8_t evt_presence(struct mgmt_nrf24_header *mhdr)
{
	 ...
	 */
	g_hash_table_insert(peer_bcast_table, peer->mac_str, peer);
done:
...
static gboolean check_timeout(gpointer key, gpointer value, gpointer user_data)
{
	...
	if (hal_timeout(hal_time_ms(), peer->time_last_bcast,
							BCAST_TIMEOUT) > 0) {
		log_info("Peer %s timedout.", peer->mac_str);
		return TRUE;
	}
