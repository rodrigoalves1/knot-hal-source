...
/* Computes p_result = p_product % curve_p.
   See algorithm 5 and 6 from www.isys.uni-klu.ac.at/PDF/2001-0126-MT.pdf */
...
static void omega_mult(uint8_t *p_result, uint8_t *p_right)
{
    ...
    p_result[16 + NUM_ECC_DIGITS] = vli_add(p_result +
     12, p_result + 12, p_right); /* 2^96 + 1 */
    p_result[17 + NUM_ECC_DIGITS] = 0;
    p_result[18 + NUM_ECC_DIGITS] = 0;
    p_result[19 + NUM_ECC_DIGITS] = 0;
    p_result[20 + NUM_ECC_DIGITS] = vli_add(p_result +
     16, p_result + 16, p_right); /* 2^128 + 2^96 + 1 */
    p_result[21 + NUM_ECC_DIGITS] = 0;
    p_result[22 + NUM_ECC_DIGITS] = 0;
    p_result[23 + NUM_ECC_DIGITS] = 0;
    if(vli_sub(p_result + 4, p_result + 4, p_right))
    /* 2^128 + 2^96 - 2^32 + 1 */
